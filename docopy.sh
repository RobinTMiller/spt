...
SPT_FILES=${SPT_FILES:?Please define SPT_FILES with list of files to copy to each build system!}
...
RGITHUB="/var/tmp/romiller/GitHub/spt"
##RGITHUB="/var/tmp/romiller/GitHub/"
...
do
    ...
    ##scp -rp ${GITHUB} ${OSUSER}@${OSHOST}:${RGITHUB}/
    scp -p ${SPT_FILES} ${OSUSER}@${OSHOST}:${RGITHUB}/
    # Note: The actual spt building is done separately.
done
