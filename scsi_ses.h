...
/*
 * HGST/WDC SES Page 4 Specific Definitions:
 */
#define SES_PP_FAN_ELEMENTS	3
#define SES_CP_FAN_ELEMENTS	4
/*
 * Note WRT MM Value, from Lee: 
 *  But, we only report 2 distinct PWM values.
 *  One PWM value applies to the 2 IOM fans. 
 *  And, the other PWM value applies to the 4 big fans.
 *
 * Notes from Brian: 12/13/2017
 *  There are 6 fans in MM but only 2 PWM settings.
 *  1 PWM for the 4 systems fans and 1 PWM for the 2 IOM fans.
 *  We don’t currently count the PSUs which we may control, so +2 PWM for the 2 PSU FANs.
 *  
 * Also Note: This is internal information, not exposed to customers.
 */
#define SES_MM_FAN_ELEMENTS	4
#define SES_MP_FAN_ELEMENTS	16

/* Valid for MP and MM dual-IOM enclosures. */
#define SES_DUAL_MODE		0x0D
#define SES_SINGLE_MODE		0x05

/* CP = Castle Peak */
#define SES_CP_SLOT_A		0xAAAA
#define SES_CP_SLOT_B		0xBBBB

/* MM = Mount Madonna */
#define SES_MM_SLOT_A		0xAAAA
#define SES_MM_SLOT_B		0xBBBB

/* MP = Mission Peak */
#define SES_MP_SLOT_A		0x0A
#define SES_MP_SLOT_B		0x0B

#define OFFLINE_REASON_ENCLOSURE_NOT_OFFLINE    0x00000000
#define OFFLINE_REASON_ILLEGAL_SLED_CFG         0x00000001
#define OFFLINE_REASON_UNABLE_READ_ENCLVPD      0x00000002
#define OFFLINE_REASON_THERMAL_FAULT            0x00000004

typedef enum  _PowerSeqStartup {
    POWERSEQ_LOW_INITIAL,
    POWERSEQ_LOW_ENABLED_PSUS,
    POWERSEQ_LOW_START_SECONDARY,
    POWERSEQ_HIGH_ON
} PowerSeq_t;

/*
 * PikesPeak Specific Definitions:
 */
typedef struct ses_pp_page4_plat_spec {
    uint8_t	fanPWM[SES_PP_FAN_ELEMENTS];	/* Fan PWM.			    [64-66] */
    uint8_t	reserved_bytes_67_127[61];	/* Reserved.			   [67-127] */
} ses_pp_page4_plat_spec_t;

/*
 * MissionPeak Specific Definitions:
 */
typedef struct ses_mp_page4_plat_spec {
    uint8_t	fanPWM[SES_MP_FAN_ELEMENTS];	/* Fan PWM.			    [64-79] */
#if defined(_BITFIELDS_LOW_TO_HIGH_)
    bitfield_t					/* 				       [80] */
	singleOrDual	: 4,			/* Single or Dual IOM.		     (b0:3) */
	slotAorB	: 4;			/* Slot A or B.			     (b4:7) */
#elif defined(_BITFIELDS_HIGH_TO_LOW_)
    bitfield_t					/* 				       [80] */
	slotAorB	: 4,			/* Slot A or B.			     (b4:7) */
	singleOrDual	: 4;			/* Single or Dual IOM.		     (b0:3) */
#endif /* defined(_BITFIELDS_LOW_TO_HIGH_) */
    uint8_t	reserved_bytes_81_127[47];	/* Reserved.			   [81-127] */
} ses_mp_page4_plat_spec_t;

/*
 * CastlePeak Specific Definitions:
 */
typedef struct ses_cp_page4_plat_spec {
    uint8_t	fanPWM[SES_CP_FAN_ELEMENTS];	/* Fan PWM.			    [64-67] */
    uint8_t	reserved_bytes_68_127[60];	/* Reserved.			   [68-127] */
} ses_cp_page4_plat_spec_t;

/*
 * Mount Madonna Specific Definitions:
 */
typedef struct ses_mm_page4_plat_spec {
    uint8_t	fanPWM[SES_MM_FAN_ELEMENTS];	/* Fan PWM.			    [64-67] */
    uint8_t     singleOrDual;                   /* Single or Dual IOM.                 [68] */
    uint8_t	reserved_bytes_69_127[59];	/* Reserved.			   [69-127] */
} ses_mm_page4_plat_spec_t;
