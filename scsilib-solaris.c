 ...
 * Local Definitions:
 */
#define DEV_PATH	"/dev/rdsk"	/* Device directory prefix. */
...
os_reset_bus(scsi_generic_t *sgp)
{
    ...
    error = Inquiry(sgp->fd, sgp->dsf, sgp->debug, sgp->errlog, NULL, NULL,
		    ...
		    USCSI_RESET_ALL, sgp->timeout, sgp->tsp);
    return(error);
}
...
os_reset_device(scsi_generic_t *sgp)
{
    ...
    error = Inquiry(sgp->fd, sgp->dsf, sgp->debug, sgp->errlog, NULL, NULL,
		    inquiry_data, sizeof(inquiry_data), 0,
		    USCSI_RESET, sgp->timeout, sgp->tsp);
    return(error);
}
...
os_spt(scsi_generic_t *sgp)
{
	...
	if (siop->uscsi_status != 0) { /* Assume a SCSI CDB failure! */
	    ...
	    if (sgp->errlog == True) {
		os_perror(opaque, "SCSI request (USCSICMD) failed on %s!", sgp->dsf);
	    }
	    sgp->error = True;
	    goto error;
	}
...
DumpScsiCmd(scsi_generic_t *sgp, struct uscsi_cmd *siop)
{
    ...
    Printf(opaque, "    Control Flags ....................... uscsi_flags: %#x = %s\n",
	   siop->uscsi_flags, buf);
    Printf(opaque, "    SCSI Result Status ................. uscsi_status: %#x (%s)\n",
	   siop->uscsi_status, ScsiStatus(siop->uscsi_status));
    Printf(opaque, "    Command Timeout ................... uscsi_timeout: %d seconds\n", siop->uscsi_timeout);
    for (bp = buf, i = 0; (i < siop->uscsi_cdblen); i++) {
	bp += sprintf(bp, "%x ", (unsigned char)siop->uscsi_cdb[i]);
    }
    Printf(opaque, "    Command Descriptor Block .............. uscsi_cdb: %s(%s)\n", buf, sgp->cdb_name);
    Printf(opaque, "    CDB Length ......................... uscsi_cdblen: %u\n", siop->uscsi_cdblen);
    Printf(opaque, "    I/O Buffer Address ................ uscsi_bufaddr: %#lx\n", siop->uscsi_bufaddr);
    Printf(opaque, "    I/O Buffer Length .................. uscsi_buflen: %u (%#x)\n",
	   siop->uscsi_buflen, siop->uscsi_buflen);
    Printf(opaque, "    I/O Residual Count .................. uscsi_resid: %u (%#x)\n",
	   siop->uscsi_resid, siop->uscsi_resid);
    Printf(opaque, "    Request Sense Buffer ................ uscsi_rqbuf: %#lx\n", siop->uscsi_rqbuf);
    Printf(opaque, "    Request Sense Length ................ uscsi_rqlen: %u (%#x)\n",
	   siop->uscsi_rqlen, siop->uscsi_rqlen);
    Printf(opaque, "    Request Sense Status ............. uscsi_rqstatus: %#x (%s)\n",
...
find_scsi_devices(scsi_generic_t *sgp, char *devpath, char *scsi_name, scsi_filters_t *sfp)
{
	...
	while ((dirent = readdir(dir)) != NULL) {
		...
		/* For Solaris, we only want slice 0 to avoid duplicates! */
		int soff = (int)strlen(dirent->d_name) - 2;
		if (soff <= 0) continue; /* Should not happen! */
		if (strncmp(dirent->d_name + soff, "s0", 2)) {
		    //if (sgp->debug) {
